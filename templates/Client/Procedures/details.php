...
<div class="overflow-hidden bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
    <div class="px-4 py-6 sm:px-6 flex justify-between items-center border-b border-gray-200">
        <div>
            <h3 class="text-base font-semibold leading-7 text-gray-900">Détails de la procédure</h3>
            <p class="mt-1 max-w-2xl text-sm leading-6 text-gray-500">Informations générales sur la procédure.</p>
        </div>
        <div>
            <a href="<?= $this->Url->build(['controller' => 'Requests', 'action' => 'add', $procedure->id]) ?>" class="inline-flex justify-center rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                <i class="fa-solid fa-play mr-2 mt-1"></i> Démarrer la demande
            </a>
        </div>
    </div>
